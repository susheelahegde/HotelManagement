...
public class SwaggerConfig {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				...
				.paths(PathSelectors.regex("/hote.*")).build().useDefaultResponseMessages(false).apiInfo(apiInfo());
	}

	private ApiInfo apiInfo() {
		ApiInfo apiInfo = new ApiInfo("Spring Boot REST API", "Spring Boot REST API for Online Store", "1.0",
				"Terms of service",
				new Contact("Susheela", "https://localhost:8080/hotels",
						"Hotel management"),
				"Apache License Version 2.0",
				"https://localhost:8080/hotels");
		return apiInfo;
	}
}
